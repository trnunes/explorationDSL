...
module Explorable
  ...
  def pivot(relations)
    ...
    @original_set.each do |pair|
      # puts "PAIR: " + pair.to_s
      # puts "is relation: " + pair.second_item.is_a?(Relation).to_s
    ...
    # puts "ELEMENTS PIVOT: " + each.inspect
    # each do |item|
    #   puts "ITEM IN EACH: " + item.to_s
    # end
    each do |item|
  ...
  def refine(*filters)
    ...
    refined_items = self.select do |item|
      ...
      #   false
      # end
      # puts "RESULT FOR: " + item.id + " = " + result.to_s
      result
    end
  ...
  def correlate(origin, target, max_length = -1)
    ...
    while !(queue.empty? || (max_length == 0)) do
      ...
      # puts "CURRENT PATH: " + current_path.inspect
      # puts "LAST NODE: " + last_node.to_s
      ...
      if last_node == target
        ...
        puts "EDGES FOR " + last_node.to_s + " : " 
        edges.each{|edge| puts edge.to_s;puts}
        edges.each do |edge|
      ...
      visited << last_node
      # puts "QUEUE: " + queue.map{|p| p.map{|s| s.to_s}.join(", ")}.inspect
      # puts "PATH: " + paths.to_s
    end
    paths.each do |path| 
  ...
  def match(pair, motif)
    ...
    # puts first_item_motif.to_s + second_item_motif.to_s
    # puts pair.to_s
    ...
    # puts first_item_match.call(pair.first_item)
    # puts second_item_match.call(pair.second_item)
