  ...
  class SchemaRelation
    ...
    def image(offset=0, limit=nil)
      if meta_relation?
        ...
        return Xplain::ResultSet.new( nodes: result_graph)
      end
      Xplain::ResultSet.new( nodes: hash_to_graph(@server.image(self, offset.to_i, limit.to_i)))
    end
    ...
    def domain(offset=0, limit=-1)
      ...
      Xplain::ResultSet.new( nodes: hash_to_graph(@server.domain(self, offset, limit)))
    end
    ...
    def restricted_image(restriction, options= {})
      ...
      Xplain::ResultSet.new( nodes: result_graph)
    end
    ...
    def restricted_domain(restriction, options = {})
      ...
      if meta_relation?   
        ...
        #TODO remove the nil parameter in ResultSet.new( nodes: ..)
        return Xplain::ResultSet.new( nodes: result_graph)        
      end
      domain_nodes = hash_to_graph(@server.restricted_domain(options))
      Xplain::ResultSet.new( nodes: domain_nodes)
    end
    ...
    def group_by_domain_hash(nodes)
      ...
      images_hash.each do |key_item, related_items|
        ...
        results_hash[key_item] = related_items.map{|related_item| Xplain::Node.new(item: related_item)}
      end
      results_hash
    end
    ...
    def group_by_image(nodes)
      ...
      Xplain::ResultSet.new( nodes: result_nodes)
    end
  end
end