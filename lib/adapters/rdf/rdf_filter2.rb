module SPARQLQuery
  module SPARQLFilter   
    ...
    def build_paginated_values_clauses(items)
      offset = 0
    ...
    def build_queries
      ...
      items = @entities.to_a.sort{|e1, e2| e1.to_s <=> e2.to_s}
      values_clauses = build_paginated_values_clauses(items)

      queries = values_clauses.map do |value_clause|
        query = "PREFIX xsd: <http://www.w3.org/2001/XMLSchema#> #{first_stmt} WHERE{ #{where_stmt}. #{value_clause}} "
        query << "}"
        query
      end
      queries
